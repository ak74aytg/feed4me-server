...
const addCropDetails = async (req, res) => {
  try {
    ...
    if (harvest_date && isNaN(harvest_date.getTime())) {
      return res
        .status(400)
        .json({ error: "Invalid harvest_date format. Use YYYY-MM-DD." });
    }
    if (expiry_date && isNaN(expiry_date.getTime())) {
      return res
        .status(400)
        .json({ error: "Invalid expiry_date format. Use YYYY-MM-DD." });
    }
    ...
    if (req.files && req.files.length > 0) {
      ...
      for (const file of req.files) {
        ...
        const fileName = `${savedCrop._id}-${Date.now()}-${Math.random()
          .toString(36)
          .substring(2, 8)}${ext}`;
      ...
      savedCrop.images = imagePaths;
      await savedCrop.save();
    }
    // const cropList = savedCrop.map((crop) => {
    const { images, ...item } = savedCrop.toObject();
    const cropItem = {
      images: images ? images.map((img) => `${BASE_URL}${img}`) : [],
      ...item,
    };
    // })
...
const getCropList = async (req, res) => {
  try {
    const cropList = await CropDetails.find()
      .select(
        "farmerID name images MRP stock description category location harvest_date expiry_date minimum_order_quantity stock_status"
      )
      .sort({ _id: -1 });
    const crops = cropList.map((crop) => {
      const cropObj = crop.toObject();
      const { images, ...item } = cropObj;

      return {
        images: Array.isArray(images)
          ? images.map((img) => (img ? `${BASE_URL}${img}` : null))
          : [],
        ...item,
      };
    });
